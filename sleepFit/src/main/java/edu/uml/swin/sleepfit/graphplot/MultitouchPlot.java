...
public class MultitouchPlot extends XYPlot implements OnTouchListener
{
	...
	public boolean addSeries(XYSeries series, XYSeriesFormatter formatter)
	{
		...
		if (maxYSeriesValue.doubleValue() == minYSeriesValue.doubleValue()) maxYSeriesValue = minYSeriesValue.doubleValue() + 1.0d;
		if (maxXSeriesValue.doubleValue() == minXSeriesValue.doubleValue()) maxXSeriesValue = minXSeriesValue.doubleValue() + 1.0d;
	...
	private void scroll(float pan)
	{
		...
		//Log.d(Constants.TAG, "MinX = " + calculatedMinX + ", MaxX = " + calculatedMaxX + ", Step = " + step + ", Offset = " + offset);
		newMinX = calculatedMinX + offset;
		newMaxX = calculatedMaxX + offset;
		//Log.d(Constants.TAG, "newMinX = " + newMinX + ", newMaxX = " + newMaxX + ", maxX = " + maxXSeriesValue);
	}
