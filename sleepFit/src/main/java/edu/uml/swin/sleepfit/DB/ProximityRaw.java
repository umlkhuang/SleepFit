...
@DatabaseTable(tableName = "proximityraw")
public class ProximityRaw implements Serializable {

	private static final long serialVersionUID = 31415926535897936L;
	...
	public ProximityRaw(long mills) {
		this.createTime = new Date(mills);
		this.data = ""; 
		this.uploaded = false;
	}
	
	public ProximityRaw(long mills, ArrayList<Float> rawData) {
		this.createTime = new Date(mills);
		StringBuilder strBuf = new StringBuilder();
		int rawLen = rawData.size();
		for (int i = 0; i < rawLen; i++) {
			strBuf.append(rawData.get(i).floatValue());
	...
	public String toString() {
		...
		strBuf.append(", ").append("ProximityData = ").append(data);
